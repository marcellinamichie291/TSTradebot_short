...
const { coin, tickerSize, profitPercent } = configReader_1.getConfig("DYDX");
...
const zeroBuy = () => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        if (getPositionInfo.length) {
            ...
            const limitPrice = rounder_1.roundValue(middlePrices[0] * (1 - (profitPercent[0] - 1)), tickerSize);
        ...
        const allMiddleOrders = countOrders_1.generateMiddleOrders(recountedMiddlePrices, recountedQty, calc.coin, tickerSize, balance_1.OrderType.SELL);
    ...
    try {
        ...
        switch (allExistingOrders.length) {
            ...
            case 5:
                if (tierArray[1] === 0) {
                    try {
                        const [{ orderId: zeroSellOrderId }] = yield binance.getBuyOrder(coin);
                        ...
                        const limitPrice = rounder_1.roundValue(+entryPrice * (1 - (profitPercent[1] - 1)), // Стоит 1 у.е. Цена на профит: 1 у.е. * (1 - (1 - 1.0038)) = 1 * (1 - 0.0038) = 1 * 0.9962
                        tickerSize);
            ...
            case 4:
                if (tierArray[2] === 0) {
                    try {
                        ...
                        yield binance.cancelLimitOrderById(coin, zeroSellOrderId);
                        const [{ positionAmt, entryPrice }] = yield binance.getCoinInPositionShort(coin);
                        const limitPrice = rounder_1.roundValue(+entryPrice * (1 - (profitPercent[2] - 1)), tickerSize);
                        yield binance.putLimitOrder({
                            coin,
                            limitPrice,
                            qty: +positionAmt * -1,
            ...
            case 3:
                if (tierArray[3] === 0) {
                    try {
                        ...
                        yield binance.cancelLimitOrderById(coin, zeroSellOrderId);
                        const [{ positionAmt, entryPrice }] = yield binance.getCoinInPositionShort(coin);
                        const limitPrice = rounder_1.roundValue(+entryPrice * (1 - (profitPercent[3] - 1)), tickerSize);
                        yield binance.putLimitOrder({
                            coin,
                            limitPrice,
                            qty: +positionAmt * -1,
            ...
            case 2:
                if (tierArray[4] === 0) {
                    try {
                        ...
                        yield binance.cancelLimitOrderById(coin, zeroSellOrderId);
                        const [{ positionAmt, entryPrice }] = yield binance.getCoinInPositionShort(coin);
                        const limitPrice = rounder_1.roundValue(+entryPrice * (1 - (profitPercent[4] - 1)), tickerSize);
                        yield binance.putLimitOrder({
                            coin,
                            limitPrice,
                            qty: +positionAmt * -1,
            ...
            case 1:
                if (tierArray[5] === 0) {
                    try {
                        ...
                        yield binance.cancelLimitOrderById(coin, zeroSellOrderId);
                        const [{ positionAmt, entryPrice }] = yield binance.getCoinInPositionShort(coin);
                        const limitPrice = rounder_1.roundValue(+entryPrice * (1 - (profitPercent[5] - 1)), tickerSize);
                        yield binance.putLimitOrder({
                            coin,
                            limitPrice,
                            qty: +positionAmt * -1,
